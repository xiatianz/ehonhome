...
(function () {
    ...
    async function parseRem(src,codes,rootPath=''){
        ...
        try{
            var code=await getCode(src);
        }catch(e){
            if(src.indexOf('index.js')==-1){
                src=src.replace('.js','')+'/index.js';
                if(codes.id[src]) return codes.id[src];
                try{
                    var code=await getCode(src);
                }catch(e){
    ...
    window.addEventListener('DOMContentLoaded',function(){
     ...
     async function gse(sss){
        if(sss.getAttribute('type')=='text/cjs'){
          if(sss.hasAttribute('src')){
            var code=await build(sss.getAttribute('src'));
            runScript(code);
          }
        }
      }
      ...
      /**
       * 处理REM
       * @param {HTMLScriptElement} script_ele 
       */
      window.updateScript=function(script_ele){
        gse(script_ele);
      }

      window.installExt=installExt;
})();